...
class TMSKeyBindsScreen(parent: Screen, gameOptions: GameOptions) :
    ...
    override fun keyPressed(input: KeyInput): Boolean {
        if (isShiftKey(input.keycode)) isShiftEnabled = true
        if (this.selectedKeyBinding == null) {
    ...
    override fun keyReleased(input: KeyInput): Boolean {
        if (isShiftKey(input.keycode)) isShiftEnabled = false
...
class TMSControlsListWidget(override val parent: TMSKeyBindsScreen, client: MinecraftClient) :
    ...
    private fun rebuildKeybindingEntries(searchValue: String?) {
        ...
        scrollY = 0.0
        var lastCategory: KeyBinding.Category? = null
        if (entries.isEmpty()) {
    ...
    override fun createEntry(binding: KeyBinding, alternative: Boolean): TMSKeyBindingEntry {
