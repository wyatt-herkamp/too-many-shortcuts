	...
	public SearchFieldControlsListWidget(MinecraftClient client) {
		...
		textFieldWidget.setChangedListener(searchText -> {
			...
			if (childrenCount != lastEntryCount) {
				TooManyShortcuts.INSTANCE.log(Level.INFO, "Controls search results changed externally - recompiling the list!");
				try {
					...
					TooManyShortcuts.INSTANCE.log(Level.ERROR, "An unexpected exception occured during recompilation of controls list!");
					e.printStackTrace();
				}
			}
			...
			if (lastEntryCount <= 1) {
				MutableText noResultsText = Text.translatable(TooManyShortcuts.MOD_ID + ".search.no_results");
