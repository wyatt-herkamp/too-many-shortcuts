...
object TMSKeyBindingUtils {
    ...
    fun debugKeyBinding(message: String, keyBinding: KeyBinding) {
        if (keyBinding is IKeyBinding) {
            TooManyShortcuts.LOGGER.debug("Debugging Key Binding {}, {}", message, keyBinding.`tms$debugString`())
        }else{
            TooManyShortcuts.LOGGER.debug("Debugging Key Binding (Not TMS Keybinding) {}, {}", message, keyBinding.translationKey)
        }
    }
}